...
TEMP_AUDIO = OUTPUT_AUDIO / "_temp_Architect_02_v2.wav"
FINAL_VIDEO = OUTPUT_VIDEO / "NeoSphere_Architect_02_v2_FULL.mp4"
...
def make_frame(t, duration, width=1080, height=1080):
    ...
    # === Этап 1: хаос (много мягких пятен)
    noise_field = (
        np.sin(3 * xv + t * 0.5) +
        np.sin(4 * yv + t * 0.6) +
    ...
    chaos_strength = max(0, min(1, 1 - progress * 2))
    chaos = np.exp(-(noise_field**2)) * chaos_strength

    # === Этап 2: схлопывание в одно пятно
    collapse_strength = max(0, min(1, (progress - 0.2) * 3))
    center_blob = np.exp(-(radius / 0.6)**2) * collapse_strength

    # === Этап 3: появление контуров
    contour_strength = max(0, min(1, (progress - 0.4) * 3))
    contour = np.sin(6 * xv) * np.sin(6 * yv) * contour_strength * 0.2

    # === Этап 4: формирование круга
    circle_strength = max(0, min(1, (progress - 0.6) * 3))
    circle = np.exp(-((radius - 0.3)**2) / 0.01) * circle_strength

    # пульсация
    pulse = 0.5 + 0.5 * np.sin(2 * np.pi * 1.1 * t)
...
def build_full_session():
    ...
    print(f"[OK] Architect 02 v2 ready: {FINAL_VIDEO}")
