...
TEMP_AUDIO = OUTPUT_AUDIO / "_temp_Architect_02_v4.wav"
FINAL_VIDEO = OUTPUT_VIDEO / "NeoSphere_Architect_02_v4_FULL.mp4"
...
def make_frame(t, duration, width=1080, height=1080):
    ...
    contour = np.sin(6 * xv) * np.sin(6 * yv) * contour_strength * 0.12

    # --- ПЕРЕХОД В ЧИСТУЮ ГЕОМЕТРИЮ ---
    clean_strength = max(0, min(1, (progress - 0.7) * 4))

    # --- ИДЕАЛЬНОЕ КРУГЛОЕ СВЕЧЕНИЕ ---
    perfect_glow = np.exp(-(radius / 0.55)**2) * clean_strength * 1.1

    # --- ТЁМНЫЙ КРУГ ---
    circle_strength = max(0, min(1, (progress - 0.62) * 3))
    circle_radius = 0.31
    edge_softness = 0.015
    ...
    # --- ЗАТУХАНИЕ ВСЕГО ЛИШНЕГО ---
    chaos *= (1 - clean_strength)
    contour *= (1 - clean_strength)
    center_blob *= (1 - clean_strength)

    # --- ПУЛЬС ---
    pulse_rate = 1.1 - 0.35 * progress
    pulse_depth = 0.4 - 0.2 * progress
    ...
    intensity = intensity * (1 - dark_circle * 0.95)
...
def build_full_session():
    ...
    print(f"[OK] Architect 02 v4 ready: {FINAL_VIDEO}")
