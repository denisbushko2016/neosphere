...
TEMP_AUDIO = OUTPUT_AUDIO / "_temp_Architect_02_v1.wav"
FINAL_VIDEO = OUTPUT_VIDEO / "NeoSphere_Architect_02_v1_FULL.mp4"
...
def generate_audio():
    ...
    stages = [
        # 0–2 мин — поле (размытое)
        (120, 240, 11.0, 10.0, 0.08, 0.16, 0.003, 0.010, 0.010, 0.09, 0.08),

        # 2–5 мин — контуры появляются
        (180, 240, 10.0, 9.2, 0.16, 0.24, 0.010, 0.015, 0.014, 0.08, 0.07),

        # 5–8 мин — структура усиливается
        (180, 240, 9.2, 8.8, 0.24, 0.26, 0.015, 0.015, 0.012, 0.07, 0.065),

        # 8–12 мин — фиксация (выбор)
        (240, 240, 8.8, 9.5, 0.26, 0.22, 0.015, 0.010, 0.010, 0.065, 0.08),
    ]
...
def make_frame(t, duration, width=1080, height=1080):
    ...
    # Этап 1: размытое поле
    blur = np.exp(-(radius / 0.8)**2)

    # Этап 2: появляется сетка
    grid_strength = max(0, min(1, (progress - 0.2) * 2))
    grid = np.sin(6 * xv) * np.sin(6 * yv)

    # Этап 3: формируется круг
    circle_strength = max(0, min(1, (progress - 0.5) * 2))
    circle = np.exp(-((radius - 0.3)**2) / 0.01)

    # Пульсация (собранная, не мягкая)
    pulse = 0.5 + 0.5 * np.sin(2 * np.pi * 1.2 * t)

    intensity = (
        0.05 +
        blur * (1 - grid_strength) * 0.6 +
        grid * grid_strength * 0.2 +
        circle * circle_strength * 0.9
    ) * pulse
...
def build_full_session():
    ...
    print(f"[OK] Architect 02 ready: {FINAL_VIDEO}")
